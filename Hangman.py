...
#  DESCRIPTION:  Monty Python hangman games
...
def hangman_instructions():
    ...
    hangman_instructions = instructions.read()
    print(hangman_instructions)
    instructions.close()
    main()
...
def main():
    ...
    words = ("CAMELOT", "ROBIN", "MINSTREL", "CASTLE", "HOLY GRAIL", "MONTY PYTHON", "KING ARTHUR", "BRITISH", "FLYING CIRCUS", "LIFE OF BRIAN", "THE MEANING OF LIFE")
    print('H A N G M A N')
    missedLetters = ''
    correctLetters = ''
    ...
    while True:
        ...
        if len(missedLetters) == len(hangmanPics) -1:
            displayBoard(hangmanPics, missedLetters, correctLetters, secretWord)
            print('You are out of guesses.  The secret words was ' + secretWord + '.  You lose!')
            gameIsDone = True
    if gameIsDone:
        if playAgain():
            main()
        else:
            print("I hope you enjoyed playing Hangman.")
            menu()
